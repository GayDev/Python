...
Решение на языке Python 3.
 Автор: Константин Поляков, 2013-2016
...
resList = [Info("",0)]*3
def insert ( res0, name0 ):
  global resList
  newEl = Info(name0, res0)
  if name0 == resList[0].name:
    if res0 > resList[0].res: resList[0] = newEl
  elif name0 == resList[1].name:
    if res0 > resList[0].res:
      resList[1] = resList[0]
      resList[0] = newEl
    elif res0 > resList[1].res: resList[1].res = res0
  elif res0 > resList[0].res:
    resList[2] = resList[1]
    resList[1] = resList[0]
    resList[0] = newEl
  elif res0 > resList[1].res:
    resList[2] = resList[1]
    resList[1] = newEl
  elif res0 > resList[2].res:
    resList[2] = newEl
...
for i in range(3):
    print("%d место. %s (%d)" % (i+1, resList[i].name, resList[i].res))
